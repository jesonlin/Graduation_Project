...
public class DataUtils {
	...
	public static void get_AllMusicListFDB(Context context) {
		System.out.println("=======进入从数据库加载音乐到本地列表");
		...
		while (cursor.moveToNext()) {// cursor往后移动直到最后一行
			...
			System.out.println("============DB-allmusiclist" + name + path
					+ artist);
		}
		cursor.close();
		db.close();
	...
	public static void updata_all_list_when_SDdelete(HashMap<String, Object> map) {
		Log.e("DataUtils", "进入更新播放列表及本地控件");
		Log.e("DataUtils", "所有音乐列表=" + MyApplication.getAllMusicList().size());
		Log.e("DataUtils", "播放音乐列表=" + MyApplication.getPlayMusicList().size());
		Log.e("DataUtils", "最新欢音乐列表="
				+ MyApplication.getFavoriteMusicList().size());
		Log.e("DataUtils", "放松音乐列表=" + MyApplication.getRelaxMusicList().size());
		Log.e("DataUtils", "学习音乐列表=" + MyApplication.getStudyMusicList().size());
