...
public class ShakeService extends Service {
				...
				.setOnSilencelistener(new ShakeListener.OnSilenceListener() {
					...
					public void onSilence() {
						...
						if (MyApplication.playStatus == 1) {
							PlayService.pause();
							Log.i("ShakeService", "=======playstatus="
									+ MyApplication.playStatus);
							if (PlayMusicActivity.isUploaded) {
								PlayMusicActivity.playButton
										.setBackgroundResource(R.drawable.play_normal);
							}
						}
					}
				});
	...
	void startChangeMusic() {
		...
		// 判断播放音乐界面的那个Activity是否加载过，只有加载过才可以使用里面的next方法
		if (PlayMusicActivity.isUploaded == true) {
			Log.e("ShakeService", "播放界面已经加载过了");
			...
			if (MyApplication.getPlayMusicList() != null) {
				...
				if (MyApplication.getPlayMusicList().size() > 1) {
					...
					// 播放列表歌曲数目为1
					PlayUtils.turnToPlay(MyApplication.getPlayMusicList()
							.get(0), this);
				} else {
