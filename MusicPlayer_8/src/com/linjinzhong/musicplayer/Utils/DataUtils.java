...
public class DataUtils {
	...
	public static void update_AllMusicDataTDB(ContentResolver resolver,
		...
		while (cursor.moveToNext()) {
			...
			musicList.add(new MusicDemo(name, path, artist, song_id, album_id));
		}
		...
		if (!musicList.isEmpty()) {// 如果SD卡上有音乐
			for (Iterator<MusicDemo> iterator = musicList.iterator(); iterator
				...
				values.put("song_id", demo.getSong_id());
				values.put("album_id", demo.getAlbum_id());
				Log.e("DataUtils", "插入数据库的记录：" + demo.toString());
	...
	public static void get_AllMusicListFDB(Context context) {
		...
		Cursor cursor = db
				.rawQuery(
						"select name,path,artist,song_id,album_id from all_music",
						null);
		while (cursor.moveToNext()) {// cursor往后移动直到最后一行
			String name = cursor.getString(cursor.getColumnIndex("name"));
			String path = cursor.getString(cursor.getColumnIndex("path"));
			String artist = cursor.getString(cursor.getColumnIndex("artist"));
			String song_id = cursor.getString(cursor.getColumnIndex("song_id"));
			String album_id = cursor.getString(cursor
	...
	public static void get_PlayMusicListFDB() {
		...
		Cursor cursor = db.rawQuery(
				"Select name,path,artist,song_id,album_id from play_music",
				null);
		while (cursor.moveToNext()) {
			String name = cursor.getString(cursor.getColumnIndex("name"));
			String path = cursor.getString(cursor.getColumnIndex("path"));
	...
	public static void get_FavoriteMusicListFDB() {
		...
		Cursor cursor = db
				.rawQuery(
						"Select name,path,artist,song_id,album_id from my_favorite_music",
						null);
		while (cursor.moveToNext()) {
			String name = cursor.getString(cursor.getColumnIndex("name"));
			String path = cursor.getString(cursor.getColumnIndex("path"));
	...
	public static void get_RelaxMusicListFD() {
		...
		SQLiteDatabase db = dbHelper.getReadableDatabase();
		Cursor cursor = db.rawQuery(
				"Select name,path,artist,song_id,album_id from my_relax_music",
				null);
		while (cursor.moveToNext()) {
			String name = cursor.getString(cursor.getColumnIndex("name"));
			String path = cursor.getString(cursor.getColumnIndex("path"));
	...
	public static void get_StudyMusicListFD() {
		...
		SQLiteDatabase db = dbHelper.getReadableDatabase();
		Cursor cursor = db.rawQuery(
				"Select name,path,artist,song_id,album_id from my_study_music",
				null);
		while (cursor.moveToNext()) {
			String name = cursor.getString(cursor.getColumnIndex("name"));
			String path = cursor.getString(cursor.getColumnIndex("path"));
	...
	public static void upData_Lists(Context context, String table_name) {
		...
		for (Iterator<HashMap<String, Object>> iterator = musicList.iterator(); iterator
			...
			values.put("song_id", (String) hashMap.get("song_id"));
			values.put("album_id", (String) hashMap.get("album_id"));
	 ...
	 * 更新数据库中的所有音乐列表
	 */
	public static void upData_allmusicList() {
